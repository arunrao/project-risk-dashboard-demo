...
- **Gemini Model:** `gemini-2.5-flash` for per-row analysis (speed + cost). `gemini-2.5-pro` for portfolio summary (quality).
- **API Key:** Read from `process.env.GEMINI_API_KEY`. Add to `.env.local`. Never hardcode.
